    ...
    public class GameClient
    {
        ...
        public void HandlerPacket30(byte[] data, int index, Crypto crypto,byte[] encryptedData)
        {
            ...
            switch (code)
            {
                ...
                case OpCodes.OPCODE_DATA_MAIL_GET:
                    ...
                    SendPacket30(OpCodes.OPCODE_DATA_MAIL_GET_NEWMAIL_HEADER, BitConverter.GetBytes(swap16((ushort) 1)));
                    break;
                case OpCodes.OPCODE_DATA_MAIL_GET_MAIL_BODY:
                   ...
                   // Log.LogData(fakeMail,0xFFFF,this.index,"Fake Mail Data",0,0);
                    ...
                    uint Mailid = swap16(BitConverter.ToUInt16(argument, 6));
                    Console.WriteLine("Mail ID " + Mailid);
                    //SendPacket30(OpCodes.OPCODE_DATA_MAIL_GET_MAIL_BODY_RESPONSE, CreateFakeMailHeader(5));
                    SendPacket30(OpCodes.OPCODE_DATA_MAIL_GET_MAIL_BODY_RESPONSE, CreateFakeMail(5));
        ...
        public byte[] CreateFakeMailHeader(uint messageid)
        {
            ...
            */
            ...
            /*while (accountID.Count<4)
            {
                accountID.Add(0x00);
            }*/
            
            List<byte> RECaccountID = BitConverter.GetBytes(swap32(55828740)).ToList();
            /*while (RECaccountID.Count<4)
            {
                RECaccountID.Add(0x00);
            }*/
            
            List<byte> face = shift.GetBytes("#Ver = 0 #FACE = 17665").ToList();
            while (face.Count<16)
            {
                face.Add(0x00);
            }
            List<byte> title = shift.GetBytes("BASHA").ToList();
            while (title.Count<128)
            {
                title.Add(0x00);
            }
            List<byte> body = shift.GetBytes(" this is a test mail SIR").ToList();
            ...
            List<byte> date = BitConverter.GetBytes(swap32((uint) t.TotalSeconds)).ToList();
            /*while (date.Count<6)
            {
                date.Add(0x00);
            }*/
          ...
          //  m.Write(BitConverter.GetBytes(0),0,4);
           ...
           // m.Write(BitConverter.GetBytes(0),0,2);
            m.Write(RECaccountID.ToArray(),0,RECaccountID.Count);
            //m.Write(BitConverter.GetBytes(0),0,2);
