...
namespace FragmentServerWV
{
    public class GameClient
    {
        ...
        {
            Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
            Encoding shift = Encoding.ASCII;
            
            List<byte> messageID = BitConverter.GetBytes(swap32(messageid)).ToList();
           
            
            List<byte> sender = shift.GetBytes("PS3").ToList();
            ...
            List<byte> receiver = shift.GetBytes("ZACKMON").ToList();
            ...
            List<byte> title;
            if (messageid == 1)
                title = shift.GetBytes("First Mail").ToList();
            else
                title = shift.GetBytes("Second Mail").ToList();
        ...
        {
            Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
            Encoding shift = Encoding.GetEncoding("SHIFT-JIS");

            List<byte> face;
            if (messageid == 1)
                face = shift.GetBytes("#VER = 0 #FACE = 17665").ToList();
            else
                face = shift.GetBytes("#VER = 0 #FACE = 17664").ToList();
            ...
            List<byte> body;
            if (messageid == 1 )
                body= shift.GetBytes(" this is a test mail Body ").ToList();
            else
                body= shift.GetBytes(" Second Body").ToList();
